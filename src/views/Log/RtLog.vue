    ...
    <div class="app-container">
        <el-table :data="tableData" style="width: 100%" border>
            ...
            <el-table-column prop="destinationDnsDomain" label="目标DNS域名" width="200">
                <template #default="{ row }">
                    {{ row.destinationDnsDomain || '-' }} <!-- 处理可能为空的字段 -->
                </template>
            </el-table-column>
...
import { getAllRtLog } from '../../api/apiForLog/logApi.js'; // 导入新的数据获取函数
...
const fetchData = async () => {
    try {
        const response = await getAllRtLog(currentPage.value, pageSize.value);
